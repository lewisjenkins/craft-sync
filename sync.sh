...
usage() {
    echo "Usage: sync.sh --pull <assets|assets:handle|db> --from <environment> [--dry-run] [--delete]"
    exit 1
}
...
# 4. Validate remote Craft installation
...
# 5. Validate local Craft installation
...
pull_db() {
    ...
    php "$SCRIPT_DIR/craft" db/restore $LOCAL_STORAGE/backups/$REMOTE_BACKUP --drop-all-tables \
